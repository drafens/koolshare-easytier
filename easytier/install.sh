...
install_now(){
	...
	# Permissions
	chmod 755 /koolshare/bin/easytier-core >/dev/null 2>&1
	chmod 755 /koolshare/bin/easytier-cli >/dev/null 2>&1
	...
	# re-enable
	if [ "${ENABLE}" = "1" ] && [ -f "/koolshare/scripts/easytier_config.sh" ];then
