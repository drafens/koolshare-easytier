...
var defaultTomlConfig = '';          // 默认TOML配置模板
...
function loadDefaultConfig() {
	$.ajax({
		url: '/res/easytier/template.toml',
		type: 'GET',
		cache: false,
		dataType: 'text',
		success: function(response) {
			defaultTomlConfig = response;
		},
		error: function() {
			console.warn('Failed to load /res/easytier/template.toml');
		}
	});
}

// 从 dbus 加载配置数据并应用到 UI
